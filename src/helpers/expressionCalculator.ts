...
function generateArrFromStr(str: string) {
  ...
  const lastSymbolIsNotNumber = /[0-9]/.test(nakedStr[nakedStr.length - 1])
  nakedStr = lastSymbolIsNotNumber ? nakedStr : nakedStr.splice(0, nakedStr.length - 1)
...
const doCalcTemp = (expr: string): string => {
  let calculation = expr.split('')
  debugger
  ...
  return String(Math.round(+calculation * 1000) / 1000)
}
...
export default { doCalc, expressionCalculator }
